...
def QSOrder(fastas, nlag=30, w=0.1, **kw):
    ...
    df_mFeats = pd.DataFrame(ls_mFeats, columns=header)
    status = True
    return df_mFeats,status
def myfun_calcQsOrder(fastafile, nlag):
    try:
        d_allParas
    except:
        ...
        if len(d_allParas)==1:
            d_allParas = globSet.getGlobParas()
    nlag = d_allParas['QsOrder']
    fastas = readFasta.readFasta(fastafile)
    status = False
    df_mFeats,status = QSOrder(fastas, nlag)
    return df_mFeats,status