...
- 对于两种不同类型的Fasta文件，该软件具有提取22 种蛋白质数值特征的能力（考虑到氨基酸序列长度常有差异，有序列等长要求的数值特征暂未被提取）。该软件也支持评估568种候选约化方案中的具有更好分类能力的前N个方案，然后计算对应的数值特征。为了增强iProps的灵活性和可扩展性，特征评估阶段允许用户上传其他数值特征文件用于比较和组合。  
- 为了在分类任务中利用各种类型的蛋白质数值特征，iProps提供了一种特征组合策略，该策略可以在一个特征向量中组合多达四个单独的特征。此外，该软件使用机器学习分类器对每个计算特征的分类性能进行初始评估。等待所有特征被评估后，iProps会依据性能指标数据排序，以确定具有最佳分类预测性能的数值特征。  
- 随后，iProps提供了一个自动机器学习工具来搜索适应上述最佳数值特征的分类器。最后，iProps的模型解释模块可以解释蛋白质类别预测的过程。该模块可以生成23个图表，从多个角度解释分类器的决策过程。此外，iProps是一个可免费访问的开源计算平台。它提供了一个用户友好的界面设计，允许轻松导航，即使没有编程经验的人也可以无缝使用它。
